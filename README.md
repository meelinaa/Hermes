...
| **Hermes.IntegrationTests**                            | **Integration tests** (**Docker** + Testcontainers **MySQL 8.4**, EF migrations, `**WebApplicationFactory`**): `**/health/live**` & `**/health/ready**` (+ readiness when DB stops), `**/api/v1/auth**` (login, refresh rotation, replay, validation), JWT bearer edge cases via protected routes, `**/api/v1/users**` (register, CRUD-style flows, profile password change with **wrong** `currentPassword` → **400** + typed problem, cross-user **403**, **401**/**404**/**400** where applicable), `**/api/v1/users/news`** (full CRUD + **401**/ **403**/ **404**/ **400**), `**POST …/notification-logs`** (happy path + **401**/ **403**/ **400**). Trait `**Integration=Docker`** on these tests. |
...
- **E-mail verification operations**: The **API** and **Blazor** profile flow (request code, modal, six-digit confirmation) are implemented. **Docker integration tests** do not yet cover the **`/users/verify/**` routes** or real inbox delivery—validate SMTP and mail jobs in your environment.
