...
| **Hermes.WebFrontend** / **Hermes.WebFrontend.Client** | **Blazor** web app (host + WebAssembly client project). Currently the **shell** of the UI; full login and settings flows are still to be built. |
...
- **Frontend not feature-complete**: Blazor projects exist, but end-to-end **login**, **token storage/refresh**, and **settings UI** wired to the API are still to be finished.
- **Tests**: Not yet a major focus; **unit tests** for services and **integration tests** for API + database are natural next steps once behaviors stabilize.

---

## Roadmap (high level)

1. **Frontend**: Implement authentication against the API (access + refresh), protected routes, and forms for CRUD on news profiles; align with existing DTOs and validation errors.
