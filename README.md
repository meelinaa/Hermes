...
- **Integration host scope**: `**Hermes.IntegrationTests`** (Docker + MySQL) builds and drives **`Hermes.Api`** only—**health/readiness**, `**/api/v1/auth`** (login, refresh, replay, **logout**), **JWT** rejection paths, **users** (incl. verification routes), **news CRUD**, and **notification log POST**. It does **not** start **`Hermes.Worker`**; Hangfire-driven schedules and background jobs are covered by **`Hermes.UnitTests`** and production-style runs of the worker.
