...
public sealed class NewsFeedApiClient(HttpClient httpClient) : INewsFeedApiClient
{
    ...
    public async Task<ApiResult<IReadOnlyList<NewsArticleDto>>> GetPreviewArticlesAsync(NewsPreviewRequestDto request, CancellationToken cancellationToken = default)
    {
        try
        {
            HttpResponseMessage response = await httpClient.PostAsJsonAsync("api/v1/news/preview", request, cancellationToken).ConfigureAwait(false);
            ...
            IReadOnlyList<NewsArticleDto>? articles = await response.Content.ReadFromJsonAsync<IReadOnlyList<NewsArticleDto>>(cancellationToken: cancellationToken).ConfigureAwait(false);
