...
public sealed class AuthTokenStore(ILocalStorageService localStorage)
{
    ...
    /// <summary>Gets the current in-memory JWT access token.</summary>
    public string? AccessToken { get; private set; }

    /// <summary>Gets the current in-memory JWT refresh token.</summary>
    ...
    public async Task EnsureLoadedFromStorageAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            // During static server-side rendering (SSR), JavaScript interop (localStorage) is unavailable.
            _loaded = false;
        }
        catch (Exception)
        {
    ...
    public async Task TouchActivityAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            // Gracefully ignore storage write failures during SSR.
        }
    }
    ...
    public async Task ClearAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            // Gracefully ignore storage remove failures during SSR.
        }
